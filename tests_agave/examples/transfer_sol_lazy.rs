//! SVM-level integration test for the Zignocchio Transfer SOL Lazy program under
//! solana-program-test.
...
#[tokio::test]
async fn test_transfer_sol_lazy_happy() {
    let pid = program_id();
    ...
    program_test.add_program("transfer-sol-lazy", pid, None);
    ...
    assert!(result.is_ok(), "transfer-sol-lazy should succeed: {:?}", result);
