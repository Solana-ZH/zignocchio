//! SVM-level integration test for the Zignocchio Transfer Owned benchmark.
//!
//! Prerequisites: `zig build -Dexample=transfer-owned` must be run before these tests.
...
fn setup_mollusk() -> Mollusk {
    let elf_path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("zig-out")
        .join("lib")
        .join("transfer-owned.so");
    let elf = std::fs::read(&elf_path).unwrap_or_else(|error| {
        panic!(
            "failed to read sBPF artifact at {}: {}. Run `zig build -Dexample=transfer-owned` first.",
            elf_path.display(),
            error
        )
    });
...
fn test_transfer_owned_happy() {
    ...
    assert!(
        ...
        "transfer-owned should succeed: {:?}",
        result.program_result
    );

    let source_post = &result.resulting_accounts[0].1;
    let destination_post = &result.resulting_accounts[1].1;
    assert_eq!(source_post.lamports, 0);
