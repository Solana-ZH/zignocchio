//! SVM-level integration test for the Zignocchio Transfer SOL Lazy program.
...
fn setup_mollusk() -> Mollusk {
    let elf_path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("zig-out")
        .join("lib")
        .join("transfer-sol-lazy.so");
    let elf = std::fs::read(&elf_path).unwrap_or_else(|error| {
        panic!(
            "failed to read sBPF artifact at {}: {}. Run `zig build -Dexample=transfer-sol-lazy` first.",
            elf_path.display(),
            error
        )
    });
...
#[test]
fn test_transfer_sol_lazy_happy() {
    ...
    assert!(
        ...
        "transfer-sol-lazy should succeed: {:?}",
...
#[test]
fn test_transfer_sol_lazy_zero_amount_fails() {
    let mollusk = setup_mollusk();
    let from = Pubkey::new_unique();
    let to = Pubkey::new_unique();
